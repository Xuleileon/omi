...
def get_speech_sample_signed_urls(paths: List[str]) -> List[str]:
    ...
    Returns:
        List of signed URLs
    """
    if not paths or not speech_profiles_bucket:
        return []
    bucket = storage_client.bucket(speech_profiles_bucket)
    signed_urls = []
    for path in paths:
...
def download_speech_profile_bytes(path: str) -> bytes:
    ...
    Returns:
        Audio bytes (WAV format), or None if bucket not configured
...
def delete_speech_profile_blob(path: str) -> bool:
    ...
    Returns:
        True if deleted, False if not found or bucket not configured
    """
    if not speech_profiles_bucket:
        return False
